...
hard dose six and worst-case error $81/256<1/3$.
...
\[
 \sqrt{\min\left\{{m\choose k},{m\choose r-k}\right\}}\max|c_S|
...
\[
 \left|\sum_{|S|=r}c_S\wh f(S)\right|
 \le\frac12\max|c_S|
       \sum_{k=0}^r{D\choose k}{D\choose r-k}m^{\min(k,r-k)/2}.
\]
...
\begin{equation}\label{eq:collection}
 |\E_+f-\E_-f|
 \le\frac12\sum_{r=1}^{\min(2D,m)}{2D\choose r}m^{r/4}\delta_r.
\end{equation}
...
\eqref{eq:flatness} bounds its expectation by
$\|HD_{\phi(HG)}v\|^2/N\le1/N$. The other norm is symmetric.
...
\section{The finite question at $N=4096$}\label{sec:finite}
