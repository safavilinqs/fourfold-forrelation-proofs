...
sectors are allowed. The extension to outcome-dependent fresh batches
is an open proof obligation; the earlier adaptive cut argument did not
supply the required norm estimate.
\end{abstract}
...
constants, not these analytic steps. This package remains subject to
independent review.

\section{The parallel Fourier bound}
Write $m=4N$ for the total number of sign coordinates, and
$\chi_S(x)=\prod_{a\in S}x_a$.
A charge-$D$ bosonic probe embeds in $D$ labelled slots with alphabet
$\{0,1,\ldots,m\}$, where $0$ denotes vacuum. Symmetrizing a photon word
with enough vacuum symbols gives an isometry from the truncated Fock
...
labels occurring oddly. Purify the input, and decompose it into
orthogonal word components $\psi_a$. If $0\le E\le I$ is the acceptance
effect, then
\[
...
\[
 ...
 &\le \max_{|S|=r}|c_S|\sum_{k=0}^r {D\choose k}{D\choose r-k}
                  m^{\min(k,r-k)/2}\\
 &\le {2D\choose r}m^{r/4}\max_{|S|=r}|c_S|.
 \end{aligned}
 \tag{3}
\]
\end{lemma}
...
\]
Apply (2) to (1), and sum over the
$ {D\choose k}{D\choose r-k}$ records. Vandermonde's identity finishes
the proof.
\end{proof}
...
Set $a_s=\sgn Z_s$, $b_s=\sgn W_s$ and form
\[
...
Otherwise the cross-covariance submatrix has norm at most
$\rho\sqrt{ab/N}\le r/(2\sqrt N)$.
A product of $a$ independent Gaussian signs has Hermite degree at
least $a$. Conditional expectation on chaos degree $p$ has norm at
most $\|\rho H_{A,B}\|^p$. Pair equal degrees and apply Cauchy--Schwarz;
both sign products have $L^2$ norm one. This proves (5).
...
\[
 ...
 \tag{6}
\]
No level below four contributes.
...
mean is $\overline F=\sigma u^{\mathsf T}M(G_1)M(G_2)M(G_3)u$.
...
\]
Gaussian invariance makes the coordinate second moments of each
nonempty left or right partial chain equal; the empty chain is $u$.
...
By (7) and Chebyshev,
\[
 \mu_\sigma\{\sigma F<1/4\}\le104104000/N.
 \tag{9}
\]
Conditioning on $\sigma F\ge1/4$ changes each law in total variation
by at most this probability. For $N\ge2^{30}$ put
$\varepsilon_0=1626625/16777216$.
A correct acceptance probability $f\in[0,1]$ must satisfy
\[
 \E_{\mu_+}f-\E_{\mu_-}f
 \ge\frac13-2\varepsilon_0=\frac{3508733}{25165824}.
 \tag{10}
\]
...
\[
 ...
 &\le\frac{2q^4}{1-q},\qquad
 q=2eDN^{-1/8}8^{-1/4}<\frac{11}{25}.
 \end{split}
\]
The last inequality uses $e<11/4$ and $8^{1/4}>5/3$.
The gap is less than $14641/109375$, whereas (10) is larger by
\[
 \frac{3508733}{25165824}-\frac{14641}{109375}
       =\frac{15314842691}{2752512000000}>0 .
\]
This contradiction proves Theorem~\ref{thm:main}.

\section{What remains for adaptive batches}
For a fresh adaptive protocol, acceptance still has degree at most
$2D$: multiply batch probabilities along each classical history.
The Gaussian construction and concentration are unchanged.
The additional requirement is precisely
\[
 \left|\sum_{|S|=r}c_S\wh f(S)\right|
 ...
 \tag{11}
\]
If (11) is proved for fresh adaptive protocols, the preceding proof
establishes the original adaptive theorem with the same constants.

The parallel proof has a fixed input vector and a single effect
$0\le E\le I$ on its word space. An outcome-selected next probe does
not supply that representation on aggregate words.
The earlier fresh-cut proof asserted a common contraction after moving
cuts through the adaptive network, but did not define its domains and
maps sufficiently to establish that assertion. A product of
contractions is contractive only with compatible domains, codomains,
and tensor orientations; a general rearrangement of a tensor network
is not such a product.

This is a gap in the supplied proof, not a counterexample to (11).
The finite-size argument has a concrete norm-interface gap described
in \path{../AUDIT.md}. Neither gap is resolved by checking constants.
The present theorem concerns parallel interrogation; adaptive,
mean-dose, and quantum-memory extensions are not claimed.
\end{document}
